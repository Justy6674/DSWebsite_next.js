...
const bedQuestions = [
  ...
  {
    id: 'distress',
    question: 'How much distress do these eating episodes cause you?',
    options: [
      { value: 0, label: 'No distress' },
      { value: 1, label: 'Mild distress' },
      { value: 2, label: 'Moderate distress' },
      { value: 3, label: 'Significant distress' },
      { value: 4, label: 'Severe distress' }
    ]
  },
  {
    id: 'speed',
    question: 'Do you eat much more rapidly than normal during these episodes?',
    options: [
      { value: 0, label: 'No, normal speed' },
      { value: 1, label: 'Slightly faster' },
      { value: 2, label: 'Moderately faster' },
      { value: 3, label: 'Much faster' },
      { value: 4, label: 'Extremely fast' }
    ]
  },
  {
    id: 'fullness',
    question: 'Do you eat until feeling uncomfortably full during these episodes?',
    options: [
      { value: 0, label: 'Never' },
      { value: 1, label: 'Rarely' },
      { value: 2, label: 'Sometimes' },
  ...
  const getResultInterpretation = (score: number) => {
    if (score <= 8) {
      return {
        ...
        icon: CheckCircle,
        description: 'Your responses suggest minimal symptoms of binge eating disorder.'
      };
    } else if (score <= 14) {
      return {
        level: 'Moderate Risk',
      ...
      return {
        ...
        icon: AlertTriangle,
        description: 'Your responses suggest symptoms that warrant professional clinical assessment.'
      };
    }
  };

  const allQuestionsAnswered = bedQuestions.every(q => answers[q.id] !== undefined);
      ...
      <div className="relative min-h-[40vh] flex items-center justify-center text-center text-white bg-slate-800">
        <div className="container mx-auto px-4 relative z-10">
          ...
          <p className="text-lg md:text-xl max-w-3xl mx-auto text-slate-300">
            Binge Eating Disorder Clinical Screening Questionnaire
          </p>
        </div>
      </div>
            ...
            {!showResults ? (
              ...
              <div className="bg-slate-900 rounded-lg p-4 md:p-8">
                <h2 className="text-xl md:text-2xl font-bold mb-6 text-cream">
                  Binge Eating Disorder Screening Questions
                </h2>
                ...
                <div className="mt-8 text-center">
                  ...
                  >
                    {allQuestionsAnswered ? 'View Results' : `Answer All Questions (${Object.keys(answers).length}/${bedQuestions.length})`}
                  </button>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-6">
                <div className={`border rounded-lg p-4 md:p-6 ${result.bgColor}`}>
                  ...
                  <p className={`text-sm md:text-base ${result.color} opacity-90 leading-relaxed`}>
                    Score: {score}/20 - {result.description}
                  </p>
                </div>
