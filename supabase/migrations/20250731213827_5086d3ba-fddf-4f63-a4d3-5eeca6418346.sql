-- Phase 2: Fix RLS policies for security hardening (Fixed version)

-- First, update existing conversations with null user_id to use a default admin user
-- We'll create an admin user first, then update the conversations
UPDATE public.conversations 
SET user_id = (
  SELECT id FROM auth.users 
  WHERE email = 'admin@downscale.com.au' 
  LIMIT 1
)
WHERE user_id IS NULL;

-- If no admin user exists, we'll handle this gracefully by creating a placeholder
-- But first let's just make user_id NOT NULL after cleaning up nulls
...
-- Only allow authenticated users to manage website crawls (will be further restricted in app logic)
