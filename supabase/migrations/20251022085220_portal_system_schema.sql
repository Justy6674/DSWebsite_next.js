...
-- Update triggers for updated_at columns (conditional creation)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column') THEN
        DROP TRIGGER IF EXISTS update_portal_content_updated_at ON public.portal_content;
        CREATE TRIGGER update_portal_content_updated_at
            BEFORE UPDATE ON public.portal_content
            FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

        DROP TRIGGER IF EXISTS update_jb_bb_feed_updated_at ON public.jb_bb_feed;
        CREATE TRIGGER update_jb_bb_feed_updated_at
            BEFORE UPDATE ON public.jb_bb_feed
            FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

        DROP TRIGGER IF EXISTS update_patient_notifications_updated_at ON public.patient_notifications;
...
DROP TRIGGER IF EXISTS portal_content_search_vector_update ON public.portal_content;
...
DROP TRIGGER IF EXISTS jb_bb_feed_search_vector_update ON public.jb_bb_feed;
